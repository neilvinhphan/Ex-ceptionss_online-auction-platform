...
public class UserMenuController extends BaseController implements Initializable {
    ...
    public void handleLogout(ActionEvent event) {
        try {
            org.example.core.dto.Request logoutRequest = new org.example.core.dto.Request("LOGOUT", null);

            String jsonRequest = org.example.client.network.ClientManager.getInstance().getGson().toJson(logoutRequest);
            org.example.client.network.ClientManager.getInstance().getClient().sendRequest(jsonRequest);

        } catch (Exception e) {
            System.err.println("Lỗi khi gửi yêu cầu LOGOUT: " + e.getMessage());
        }

        // 2. Xóa data cục bộ và quay về màn Login
